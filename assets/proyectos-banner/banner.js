(function(){
  "use strict";
  const API = window.SPProjectsBanner = window.SPProjectsBanner || {};
        ...
        <section class="spb-banner__right">
          <div class="spb-banner__console" data-spb-console>
            <div class="spb-banner__screen">
              <div class="spb-banner__viewport" data-spb-viewport aria-label="Galería de proyectos. Usa el scroll, las flechas del teclado o un clic para cambiar. Doble clic para conocer la historia.">
                ...
                <div class="spb-banner__caption"><h2 data-spb-title></h2><p data-spb-subtitle></p><span class="spb-banner__discover">doble clic para descubrir</span></div>
              </div>
            </div>
  ...
  function mount(target, inputConfig){
    ...
    const config=deepClone(inputConfig || window.SP_PROJECTS_BANNER_CONFIG || {});
    ...
    const titleEl=root.querySelector('[data-spb-title]');
    ...
    function setTheme(slide){
      ...
      const rgb=accent.replace('#','').match(/.{2}/g)?.map(v=>parseInt(v,16)) || [47,110,232];
      root.style.setProperty('--spb-accent-rgb',rgb.join(','));
    ...
    function openModal(){
      const slide=slides[active]; if(!slide)return;
      modalImage.src=asset(config,slide.image);modalImage.alt=slide.alt||slide.title||'';modalTag.textContent=slide.tag||'';modalTitle.textContent=slide.title||'';modalStory.textContent=slide.story||'';
  ...
  API.autoMount=function(){document.querySelectorAll('[data-sp-projects-banner]').forEach(el=>{if(!mounted.has(el))mount(el,window.SP_PROJECTS_BANNER_CONFIG)});};
