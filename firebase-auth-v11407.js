(() => {
  ...
  function friendlyError(error) {
    ...
    const messages = {
      ...
      "auth/google-sdk-load-failed":"No fue posible cargar el acceso seguro de Google.",
      "auth/google-origin-not-authorized":`El dominio ${location.origin} no está autorizado en el cliente OAuth de Google.`,
      "auth/google-client-resolution-failed":"No fue posible obtener desde Firebase el cliente OAuth correcto para Google.",
      "auth/google-credential-rejected":"Firebase rechazó la credencial de Google. Verifique que el proveedor Google esté habilitado.",
  ...
  const GOOGLE_IDENTITY_SCRIPT = "https://accounts.google.com/gsi/client?hl=es-419";
  const GOOGLE_IDENTITY_SCRIPT_ID = "google-identity-services-firebase";
  const FIREBASE_AUTH_URI_ENDPOINT =
  ...
  let firebaseGoogleClientIdPromise = null;
  ...
  function parseFirebaseRestError(payload,status) {
    const serverMessage = String(payload?.error?.message || "").trim();
    const normalized = serverMessage.toLowerCase();
    const code = normalized.includes("unauthorized_domain")
      ? "auth/unauthorized-domain"
      : normalized.includes("operation_not_allowed")
        ? "auth/operation-not-allowed"
        : "auth/google-client-resolution-failed";
    return authFlowError(
      code,
      serverMessage
        ? `Firebase no pudo preparar el acceso con Google: ${serverMessage}`
        : `Firebase no pudo preparar el acceso con Google (HTTP ${status}).`,
      {firebaseResponse:payload,httpStatus:status}
    );
  }

  async function resolveFirebaseGoogleClientId() {
    if (firebaseGoogleClientIdPromise) return firebaseGoogleClientIdPromise;

    firebaseGoogleClientIdPromise = (async () => {
      const continueUri = new URL(location.href);
      continueUri.hash = "";
      continueUri.search = "";

      const response = await fetch(FIREBASE_AUTH_URI_ENDPOINT,{
        method:"POST",
        mode:"cors",
        cache:"no-store",
        credentials:"omit",
      ...
      if (!response.ok) throw parseFirebaseRestError(payload,response.status);

      const authUri = String(payload?.authUri || "");
      let clientId = "";
      try {
        clientId = new URL(authUri).searchParams.get("client_id") || "";
      } catch (_) {
        const match = authUri.match(/[?&]client_id=([^&]+)/i);
        clientId = match ? decodeURIComponent(match[1]) : "";
      }

      if (!clientId || !/^[0-9]+-[a-z0-9_-]+\.apps\.googleusercontent\.com$/i.test(clientId)) {
        throw authFlowError(
          "auth/google-client-resolution-failed",
          "Firebase respondió sin un cliente OAuth de Google válido.",
          {firebaseResponse:payload,authUri}
        );
      }

      return clientId;
    })().catch(error => {
      firebaseGoogleClientIdPromise = null;
      throw error;
    });

    return firebaseGoogleClientIdPromise;
  }
  ...
  function loadGoogleIdentityServices() {
    ...
    googleIdentityPreparation = new Promise((resolve,reject) => {
      let settled = false;
      let pollTimer = 0;
      let timeoutTimer = 0;
      ...
      const existing = document.getElementById(GOOGLE_IDENTITY_SCRIPT_ID)
        || [...document.scripts].find(script => script.src.startsWith("https://accounts.google.com/gsi/client"));
      ...
      script.addEventListener("load",() => {
        window.setTimeout(() => {
          if (!succeed()) fail("Google Identity Services cargó sin exponer la API de identidad.");
        },0);
      },{once:true});
  ...
  function ensureGoogleCredentialLayer() {
    let layer = document.getElementById("firebaseGoogleCredentialLayer");
    if (layer) {
      return {
        layer,
        panel:layer.querySelector(".firebase-google-card"),
        button:layer.querySelector(".firebase-google-official-button"),
        status:layer.querySelector(".firebase-google-status"),
        close:layer.querySelector(".firebase-google-close")
      };
    }

    layer = document.createElement("div");
    layer.id = "firebaseGoogleCredentialLayer";
    layer.className = "firebase-google-layer";
    layer.hidden = true;
    layer.innerHTML = `
      <section class="firebase-google-card" role="dialog" aria-modal="true" aria-labelledby="firebaseGoogleTitle">
        <button class="firebase-google-close" type="button" aria-label="Cerrar acceso con Google">×</button>
        <span class="firebase-google-eyebrow">ACCESO SEGURO</span>
        <h2 id="firebaseGoogleTitle">Continuar con Google</h2>
        <p>Seleccione la cuenta autorizada para ingresar al portal.</p>
        <div class="firebase-google-official-button" aria-label="Botón oficial de acceso con Google"></div>
        <small class="firebase-google-status" aria-live="polite">Conexión protegida por Google y Firebase.</small>
      </section>`;
  ...
  function requestGoogleIdToken(googleIdentity,clientId) {
    return new Promise((resolve,reject) => {
      const ui = ensureGoogleCredentialLayer();
      let settled = false;
      let escapeHandler = null;

      const closeLayer = () => {
        ui.layer.classList.remove("is-open","is-busy");
        window.setTimeout(() => {
          ui.layer.hidden = true;
          ui.button.replaceChildren();
        },180);
      ...
      const cancel = () => finish(
        reject,
        authFlowError("auth/cancelled-popup-request","El acceso con Google fue cancelado.")
      );

      ui.close.onclick = cancel;
      ui.layer.onclick = event => {
        if (event.target === ui.layer) cancel();
      };
      escapeHandler = event => {
      ...
      ui.status.textContent = "Seleccione una cuenta para continuar.";
      ui.button.replaceChildren();
      ui.layer.hidden = false;
      requestAnimationFrame(() => ui.layer.classList.add("is-open"));
      ...
      try {
        googleIdentity.initialize({
          ...
          callback:response => {
            ...
            ui.layer.classList.add("is-busy");
            ui.status.textContent = "Validando la cuenta con Firebase…";
        ...
        googleIdentity.renderButton(ui.button,{
          type:"standard",
          theme:"outline",
          size:"large",
          text:"continue_with",
  ...
  async function signInGoogle() {
    ...
    googleSignInPromise = (async () => {
      ...
      const [googleIdentity,clientId] = await Promise.all([
        loadGoogleIdentityServices(),
        resolveFirebaseGoogleClientId()
      ]);

      const idToken = await requestGoogleIdToken(googleIdentity,clientId);
      const credential = runtime.modules.auth.GoogleAuthProvider.credential(idToken);
      ...
      try {
        ...
        console.error("[Firebase] Google entregó una credencial, pero Firebase la rechazó.",{
          code:error?.code,
        ...
        if (error?.code === "auth/invalid-credential"
          ...
          throw authFlowError(
            ...
            "Firebase rechazó la credencial de Google. Revise que el proveedor Google continúe habilitado en Authentication.",
            {cause:error,clientId}
          );
        }
        throw error;
      }
